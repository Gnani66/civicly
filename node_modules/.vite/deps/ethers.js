...
function bitLen(n$1) {
	let len;
	for (len = 0; n$1 > _0n$3; n$1 >>= _1n$4, len += 1);
	return len;
}
/**
...
const bitSet = (n$1, pos, value) => {
	return n$1 | (value ? _1n$4 : _0n$3) << BigInt(pos);
};
/**
...
function invert(number$1, modulo) {
	if (number$1 === _0n$2 || modulo <= _0n$2) throw new Error(`invert: expected positive integers, got n=${number$1} mod=${modulo}`);
...
function tonelliShanks(P) {
	...
	let Q, S$1, Z;
	for (Q = P - _1n$3, S$1 = 0; Q % _2n$2 === _0n$2; Q /= _2n$2, S$1++);
...
function mapHashToField(key, fieldOrder, isLE$1 = false) {
	...
	const reduced = mod(isLE$1 ? bytesToNumberBE(key) : bytesToNumberLE(key), fieldOrder - _1n$3) + _1n$3;
...
var _0n = BigInt(0), _1n$1 = BigInt(1), _2n$1 = BigInt(2), _3n = BigInt(3), _4n = BigInt(4);
	...
	class Point$1 {
		...
		*/
		multiplyAndAddUnsafe(Q, a, b$1) {
			...
			const mul = (P, a$1) => a$1 === _0n || a$1 === _1n$1 || !P.equals(G) ? P.multiplyUnsafe(a$1) : P.multiply(a$1);
...
function weierstrass(curveDef) {
	...
	function isBiggerThanHalfOrder(number$1) {
		return number$1 > CURVE_ORDER >> _1n$1;
	}
	...
	*/
	static from(sig) {
		...
		if (typeof sig === "string") {
			...
			if (bytes$1.length === 65) return new Signature(_guard$3, hexlify(bytes$1.slice(0, 32)), hexlify(bytes$1.slice(32, 64)), Signature.getNormalizedV(bytes$1[64]));
...
var SigningKey = class SigningKey {
	...
	*/
	static computePublicKey(key, compressed) {
		...
		if (bytes$1.length === 32) return hexlify(secp256k1.getPublicKey(bytes$1, !!compressed));
		if (bytes$1.length === 64) {
		...
		return hexlify(secp256k1.ProjectivePoint.fromHex(bytes$1).toRawBytes(compressed));
	}
	/**
	...
	*/
	...
	toString() {
		...
		if (auths) {
			...
			output$1.push(`authorizations: [ ${outputAuths.join(", ")} ]`);
		}
		return `Transaction { ${output$1.join(", ")} }`;
	}
	/**
	...
	if (!sub) {
		...
		const listener = (log) => {
			...
			if (foundFragment) {
				...
				emit(contract, event, fragment ? contract.interface.decodeEventLog(fragment, log.data, log.topics) : [], (listener$1) => {
...
function mnemonicToEntropy(mnemonic, wordlist$1) {
	...
	const checksumMask = getUpperMask(words$1.length / 3);
...
var Mnemonic = class Mnemonic {
	...
	*/
	static entropyToPhrase(_entropy, wordlist$1) {
		return entropyToMnemonic(getBytes(_entropy, "entropy"), wordlist$1);
	}
	/**
...
function decryptKeystoreJsonSync(json, _password) {
	...
	if (params.name === "pbkdf2") {
		...
		return getAccount(data, pbkdf2(password, salt$1, count, dkLen$1, algorithm));
	}
	assert(params.name === "scrypt", "cannot be reached", "UNKNOWN_ERROR", { params });
	const { salt, N: N$2, r, p, dkLen } = params;
	return getAccount(data, scryptSync(password, salt, N$2, r, p, dkLen));
}
...
async function decryptKeystoreJson(json, _password, progress) {
	...
	if (params.name === "pbkdf2") {
		...
		const { salt: salt$1, count, dkLen: dkLen$1, algorithm } = params;
		const key = pbkdf2(password, salt$1, count, dkLen$1, algorithm);
		if (progress) {
			progress(1);
			await stall$1(0);
		}
	...
	assert(params.name === "scrypt", "cannot be reached", "UNKNOWN_ERROR", { params });
	const { salt, N: N$2, r, p, dkLen } = params;
	return getAccount(data, await scrypt(password, salt, N$2, r, p, dkLen, progress));
}
